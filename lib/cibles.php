...
/* require_once DOL_DOCUMENT_ROOT.'/axmodule/lib/mailing.functions.php'; */
...
// Search modules dirs
$modulesdir = myDolGetModulesDirs('/axmodule');
...
if ($action == 'add')
{
	...
	foreach ($modulesdir as $dir)
	{
	    ...
	    $file = $dir."/lib/".$module.".modules.php";
	    $classname = "mailing_".$module;

	    dol_syslog("File to load <".$file."> class <".$classname.">", LOG_DEBUG);
		if (file_exists($file))
		{
			require_once $file;
	...
	dol_syslog("Axmodule result <".$result.">", LOG_DEBUG);
	if ($result > 0)
	{
	  dol_syslog("Axmodule redirect: <".DOL_DOCUMENT_ROOT.'/comm/mailing/cibles.php?id='.$id, LOG_DEBUG);
	  header("Location: ".DOL_DOCUMENT_ROOT.'/comm/mailing/cibles.php?id='.$id);
	  exit;
	}
	if ($result == 0)
	{
